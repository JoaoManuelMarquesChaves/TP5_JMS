    ...
    switch(appData.state)
    {
        case APP_TCPIP_WAIT_INIT:
            ...
            else if(tcpipStat == SYS_STATUS_READY)
            {
                ...
                for(i = 0; i < nNets; i++)
                {
                    ...
                    SYS_CONSOLE_PRINT("    Interface %s on host %s - NBNS enabled\r\n", netName, netBiosName);//fait apparaitre les message
#else
        ...
        case APP_TCPIP_SERVING_CONNECTION:
        {
            if (!TCPIP_TCP_IsConnected(appData.socket))
            {
                ...
                //RemoteParamGen.FlagTcpip = false;
                break;
            }
            ...
            for(w = 0; w < wMaxGet; w += sizeof(AppBuffer))
            {
                ...
                for(w2 = 0; w2 < wCurrentChunk; w2++)
                {
                    ...
                    if(i >= 'a' && i <= 'z')
                    {
                            i -= ('a' - 'A'); // recomie les lettre minuscule en majuscule
                            AppBuffer[w2] = i;
                    }
                    else if(i == '\e')   //escape
                    {
            ...
            int16_t Size = TCPIP_TCP_GetIsReady(appData.socket);	// Get TCP RX FIFO byte count
            if ( Size != 0)
            {
                TCPIP_TCP_ArrayGet(appData.socket, AppBuffer , Size);
                if(AppBuffer[0] == '\e')   //escape
                {
                    appData.state = APP_TCPIP_CLOSING_CONNECTION;
                    SYS_CONSOLE_MESSAGE("Connection was closed\r\n");
                    Remotemode_Val(false);
